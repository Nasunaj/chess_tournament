...
class Tournament:
    """ Tournament class """
    def __init__(self, name: str, location: str, start_date: datetime,
                 end_date: datetime, rounds: int = 4):
        """
    ...
    def _generate_id(self):
        ...
        '''takes 3 first characters from name and if
        necessary completing with _ '''
        name_part = self.name.strip()[:3].lower()
        if len(name_part) < 3:
            name_part = name_part.ljust(3, '_')
        '''takes 3 first characters from location and if
        necessary completing with _'''
    ...
    def add_player(self, player: Player) -> None:
        ...
        if player in self._players:
            raise ValueError(f"Player {player.national_id} already "
                             f"exists (record for this tournament)")
        self._players.append(player)

    def generate_first_round(self) -> Round:
        """Generates a first round of the tournament in randomly
        ...
        if len(self._players) < 2:
            raise ValueError("At least 2 players are required to "
                             "generate a round")
        if len(self._players) % 2 != 0:
            raise ValueError("The number of players must be even to "
                             "generate a round")
    ...
    def _has_played_before(self, player1: Player, player2: Player) -> bool:
        """Checks if two players have already played against each other in
        previous rounds.
        :param player1(Player): first player
        :param player2(Player): second player
        :return: bool True if the players have already played together,
        False otherwise
        """
        for round in self._rounds_list:
            for match in round._matches:
                if ((match._player1 == player1
                     and match._player2 == player2)
                        or (match._player1 == player2
                            and match._player2 == player1)):
                    return True
        return False

    def _find_opponent(self, player1: Player, available_players: List[Player],
                       paired_players: List[Player]) -> Player:
        """Find an opponent for player1"""
        for player2 in available_players:
            if (player2 not in paired_players
                    and not self._has_played_before(player1, player2)):
                return player2
        return None
    ...
    def generate_next_round(self) -> Round:
        ...
        :return: the generated round
        :raises ValueError: if number of player is odd (3 or 5, 7 ,etc.) or
        ...
        if len(self._rounds_list) >= self.rounds:
            raise ValueError("All the rounds are generated for this "
                             "tournament")

        # Sort players by descending score, then randomly in case of a tie
        sorted_players = sorted(self._players)
        paired_players = []  # List to track already paired players.
        ...
        for i in range(0, len(sorted_players), 1):
            ...
            # Search an opponent for player 1
            player2 = self._find_opponent(player1, sorted_players[i + 1:],
                                          paired_players)

            if player2:
                match = Match(player1, player2)
                round._matches.append(match)
                # add player1 and player2 individually
                paired_players.extend([player1, player2])
            else:
                raise ValueError(f"Unable to pair the "
                                 f"player {player1.national_id}. he has "
                                 f"already played against all other players")
    ...
    def to_dict(self) -> dict:
        return {
            ...
            # convert datetime to ISO str. If not you can't save in.json
            "start_date": self.start_date.isoformat(),
            "end_date": self.end_date.isoformat(),
            "rounds": self.rounds,
            "players": [player.to_dict() for player in self._players],
            # record only ID list of players who participe at this tournament
            # "players": [player.national_id for player in self._players],
            "rounds_list": [round.to_dict() for round in self._rounds_list]
        }

    # return file path where the tournament is recorded
    def save_to_json(self, file_path: str = None) -> str:
        """Saves the tournament to a JSON file.
        :param file_path: path to file (str optional): file path.
        If not specified, uses data/tournaments/{name}.json.
        ...
        with open(file_path, mode="w", encoding="utf-8") as file:
            json.dump(tournament_dict, file, indent=4)
        return file_path

    '''@staticmethod is a decorator in Python that indicates the following
    method is a static method.
    A static method does not have access to the class instance
    (it has no self parameter).
    It is called directly on the class, without needing
    ...
    def load_from_json(file_path: str) -> "Tournament":
        ...
        # keys verification
        required_keys = ["name", "location", "start_date", "end_date",
                         "rounds", "players", "rounds_list"]
        for keys in required_keys:
            if keys not in tournament_data:
                raise ValueError(f"Json file invalid : missing required "
                                 f"key {keys} in tournament JSON file")
        # dates conversion
        ...
        tournament._id = tournament_data.get("id_tournament",
                                             tournament._generate_id())
        # Dictionary to stock players by their national ID.
        players_dict = {}
        # load players
        ...
        for round_data in tournament_data["rounds_list"]:
            ...
            if round_data['end_time']:
                round._end_time = datetime.fromisoformat(
                    round_data["end_time"]
                )
