...
class TestTournament(unittest.TestCase):
    ...
    def test_generate_next_round_impossible_pairing(self):
        ...
        '''second_round= self.tournament.generate_next_round()
        for match in second_round._matches:
            match.set_result(0.5,0.5)'''
    ...
    '''def tearDown(self):
        """ Clean created files"""
        if os.path.exists("data/tournaments/tournaments.json"):
            os.remove("data/tournaments/tournaments.json")'''

    def test_load_from_json(self):
        """ Tests loading a tournament from a JSON file"""
        loaded_tournament=Tournament.load_from_json("data/tournaments/tournaments.json")
