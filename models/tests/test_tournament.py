...
class TestTournament(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tournament = Tournament("Summer Tournament",
                                     "Paris", self.start_date,
                                     self.end_date, rounds=4)

        # create players for the test
        self.player1 = Player("AB00001", "NameA",
                              "FirstnameA", "1990-01-01",
                              "clubA")
        self.player2 = Player("AB00002", "NameB",
                              "FirstnameB", "1995-02-02",
                              "Club B")
        self.player3 = Player("AB00003", "NameC",
                              "FirstnameC", "1992-03-03",
                              "Club C")
        self.player4 = Player("AB00004", "NameD",
                              "FirstnameD", "1993-04-04",
                              "Club D")
    ...
    def test_first_round_not_enough_players(self):
        """Test not enough players for the first round"""
        self.tournament.add_player(self.player1)
        with self.assertRaises(ValueError):
    ...
    def test_generate_next_round(self):
        """Test the generate_next_round method """
        for player in [self.player1, self.player2, self.player3, self.player4]:
            self.tournament.add_player(player)
        self.tournament.generate_first_round()
    ...
    def test_generate_next_round_impossible_pairing(self):
        """Test the generate_next_round method impossible pairing"""
        ...
        '''This 3rd round will produce an error because each player has
        already played against all the other players.'''
        with self.assertRaises(ValueError):
            self.tournament.generate_next_round()
    ...
    def test_load_from_json(self):
        ...
        loaded_tournament = Tournament.load_from_json(
            "data/tournaments/sumpar20260801.json")
        ...
        # Note: players is private
        self.assertEqual(len(loaded_tournament._players), 2)
        # rounds_list is private
        self.assertEqual(len(loaded_tournament._rounds_list), 1)
